...
function vowelCount(str){
   ...
   strArr.forEach(function(strChar) {
    if('aeiou'.indexOf(strChar.toLowerCase()) !== -1){
        if (strChar.toLowerCase() in newObj) {
            newObj[strChar.toLowerCase()] += 1;
        } else {
            newObj[strChar.toLowerCase()] = 1;
        }
    }
...
function removeVowels(str) {
    let strArr = Array.from(str);
    let newArr = strArr.filter(function(val) {
        return 'aeiuo'.indexOf(val.toLowerCase()) === -1;
    });
    let newStr = '';
